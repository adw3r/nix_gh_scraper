...
class GitScraper:
    ...
    async def get_searching_page(
        self, query: str, searching_type: str
    ) -> curl_cffi.Response:
        params = {"q": query, "type": searching_type}
        response = await self.session.get(
            "https://github.com/search", params=params, headers=REQUEST_HEADERS
        )
        return response
...
async def retrieve_info(input_data: dict) -> list[dict]:
    ...
    if input_data.type.lower() == "repositories":
        ...
        if not results:
            raise Exception(
                f"check_extras method can`t give empty results with given {input_data = }"
            )
